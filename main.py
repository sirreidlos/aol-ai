from flask import Flask, send_from_directory, request, make_response, redirect, jsonify
import uuid
import os
...
# DATA SETUP
diagnosis_model = tf.keras.models.load_model("model/relu_unscaled.keras")
diagnosis_file = pd.read_csv('label.csv')
X = diagnosis_file.drop(columns=['diseases'])
Y = diagnosis_file['diseases']
...
def prompt():
    ...
    history[user_id].append(['user', prompt])
    ...
    gemini_response_dict = json.loads(response.text)
    print(gemini_response_dict)
    symptoms = gemini_response_dict['symptoms']
    
    symptoms_dict = {}
    for s in symptoms:
        symptoms_dict[s] = 1

    symptoms_pd = pd.DataFrame([symptoms_dict], columns=feature_names)
    symptoms_np = np.nan_to_num(symptoms_pd.to_numpy())
    prediction = diagnosis_model.predict(symptoms_np)

    res = {}
    res['symptoms'] = symptoms
    res_disease = [[0] for _ in range(len(prediction[0]))]
    # print(len(prediction[0]))
    for idx, p in enumerate(prediction[0]):
        res_disease[idx] = [p*100, label_encoder.inverse_transform([idx])[0]] 

    res_disease.sort(reverse=True)

    for idx, arr in enumerate(res_disease):
        res_disease[idx] = [f"{arr[0]}", arr[1]]
    res['diseases'] = res_disease

    print(res)
    history[user_id].append(['bot', res])
...
def get_history():
    ...
    # chat_history = [for s in history[user_id]]
